    ...
    try:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                task.save()
                task_id= task.id
                send_task_notification_email.delay(task_id=int(task_id), assigned_user_email=task.assigned_to.email)
...
def task_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            send_task_notification_email.delay(task.id, task.assigned_to.email)
            return redirect('task-list')
    else:
